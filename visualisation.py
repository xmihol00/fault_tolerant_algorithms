...
for i, name in enumerate(names_reversed):
    ...
    event_coordinates = np.array(Enumerable(events_by_names_ordered_dict[name]).where(lambda x: x[0] != "Nop") # don't plot 'Nop' events
                                                                               .select(lambda x: [x[1][0][1] * SCALE, i + 0.5]) # [x, y]
                                                                               .to_list()) # get a numpy array of x, y coordinates of events for the plot
    event_names = (Enumerable(events_by_names_ordered_dict[name]).where(lambda x: x[0] != "Nop") # don't plot 'Nop' events
                                                                 .select(lambda x: x[0])
                                                                 .to_list()) # get a list of names in the same order as the coordinates for these events
...
recieve_events = (Enumerable(events_by_names_ordered_dict.values()).select_many(lambda x: x) # select events from every person
...
def update(time, events_coordinates, events_plots, names, indices, recieve_events_coordinates, recieve_events_plots):
    for i in range(len(events_plots)):
        ...
        if event_points.shape[0] > 0 and event_points[-1, 0] > indices[i][0]: # new event was displayed, therfore its label must be displayed as well
            ...
            axis.annotate(names[i][indices[i][1]], (event_points[-1, 0] - 15, event_points[-1, 1] + label_adjustments[height_adjust]), fontweight="bold") # display the label
